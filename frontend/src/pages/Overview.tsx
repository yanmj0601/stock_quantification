...
function Overview() {
  const [data, setData] = useState<Dashboard>(fallbackDashboard);
  const [state, setState] = useState<"loading" | "ready" | "fallback">("loading");

  useEffect(() => {
    apiGet<Dashboard>("/api/dashboard")
      .then((dashboard) => {
        setData(dashboard.strategy_count ? dashboard : fallbackDashboard);
        setState("ready");
      })
    ...
    <div className="page-stack">
      <div className="status-strip">
        <span className={`pill ${state === "fallback" ? "warning" : "ok"}`}>
          {state === "loading" ? "Loading API" : state === "fallback" ? "Fallback data" : "API connected"}
        </span>
